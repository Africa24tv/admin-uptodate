            ...
            {{-- {{ dd($permission->author) }} --}}
                <tr class="text-gray-700">
                    ...
                    <td class="px-4 py-3 text-sm border text-center">
                        <div>
                            <form action="{{ route('permissions.destroy', $permission->id) }}" class="flex flex-row space-x-4" method="post">
                                @method('DELETE')
                                @csrf
                                <a class="text-blue-500 outline-none transform ease-in-out duration-200 hover:scale-125" href="{{ route('permissions.edit', $permission->id) }}"><i class="fas fa-edit"></i></a>
