...
<section class="container mx-auto p-6 font-mono">
  <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
    ...
    <div class="flex flex-row items-center bg-white">
        <div class="w-1/5">
            <select class="py-2 px-4 rounded-tl outline-none">
        ...
        <tbody class="bg-white">
            ...
            {{-- {{ dd($scroll->author) }} --}}
                <tr class="text-gray-700">
                    ...
                    <td class="px-4 py-3 text-ms font-semibold border">{{ $article->post->subject->title}}</td>
                    <td class="px-4 py-3 text-ms font-semibold border">{{ $article->post->status }}</td>

                    <td class="px-4 py-3 text-sm border text-center">
                        <div class="justify-center">
                            <form action="{{ route('articles.destroy', $article->id) }}" method="post" class="flex flex-row space-x-4">
                                @method('DELETE')
                                @csrf
                                <a class="text-blue-500 outline-none transform ease-in-out duration-200 hover:scale-125" href="{{ route('articles.show', $article->id) }}"><i class="fas fa-eye"></i></a>
                                <a class="text-blue-500 outline-none transform ease-in-out duration-200 hover:scale-125" href="{{ route('articles.edit', $article->id) }}"><i class="fas fa-edit"></i></a>
